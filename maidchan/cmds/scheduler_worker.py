...
from maidchan.offerings import get_morning_offerings_text,\
    get_morning_offerings_image, get_night_offerings_text,\
    get_night_offerings_image
from pymessenger.bot import Bot

bot = Bot(ACCESS_TOKEN)


def process_user(redis_client, recipient_id, current_mt):
    logging.info("Now processing {} . . .".format(recipient_id))
    user = redis_client.get_user(recipient_id)
    schedules = user["schedules"]
    for schedule_type, mt in schedules.iteritems():
        if schedule_type == "morning_offerings_mt":
            continue
        elif schedule_type == "night_offerings_mt":
            continue
        elif schedule_type == "japanese_lesson_mt":
            continue
...
def main():
    ...
    while True:
        ...
        if not metadata or metadata["next_mt"] < current_mt:
            ...
            metadata["morning_offering_image"] = get_morning_offerings_image()
            metadata["night_offering_text"] = get_night_offerings_text()
            metadata["night_offering_image"] = get_night_offerings_image()
